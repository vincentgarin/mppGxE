...
CheckModelGE <- function(mppData = NULL, trait, Q.eff, VCOV, par.clu = NULL,
                             plot.gen.eff = FALSE, parallel = FALSE,
                             cluster, cofactors = NULL, QTL = NULL,
                             ref.par = NULL, mppData.ts = NULL,
                             mppData.vs = NULL, EnvNames = NULL, fct = "XXX"){
  ...
  if (fct != "R2"){
    ...
    if (!(VCOV %in% c("h.err", "Env.err"))){

      stop(paste("The VCOV argument must be : 'h.err' or 'Env.err'."))
  ...
  if(!is.null(EnvNames)){
    ...
    if(!is.character(EnvNames)){

      stop("EnvNames must be a character vector")

    }

    if(length(EnvNames) != dim(trait)[2]){

      stop(paste("EnvNames must contain", dim(trait)[2], "character expressions.",
                 "As much as the number of environment or traits specified."))
